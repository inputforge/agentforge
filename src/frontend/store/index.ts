...
export const useStore = create<AppState>((set, get) => ({
  ...
  fetchTickets: async () => {
    ...
    try {
      ...
      const needAgents = tickets.filter((t) => t.agentId && (t.status === 'in-progress' || t.status === 'review'))
      await Promise.all(needAgents.map((t) => get().fetchAgentForTicket(t.id)))
    } catch (err) {
      get().addNotification({ type: 'error', message: `Failed to load tickets: ${(err as Error).message}` })
    } finally {
  ...
  moveTicket: async (ticketId, newStatus) => {
    ...
    try {
      ...
      // If moved to in-progress, backend spawns an agent — fetch it after a short delay
      if (newStatus === 'in-progress') {
        // Open the panel immediately so the agent-picker is shown
