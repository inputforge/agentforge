  ...
  switch (event) {
      ...
      if (!ticket.agentTitle && body.transcript_path && typeof body.transcript_path === 'string') {
        try {
          ...
          for (const line of lines) {
            try {
              const msg = JSON.parse(line) as { role?: string; message?: { content?: { type: string; text: string }[] } }
              if (msg.role === 'assistant' && Array.isArray(msg.message?.content)) {
                const textBlock = msg.message.content.find((b) => b.type === 'text')
                if (textBlock?.text) {
                  const firstLine = textBlock.text.split('\n')[0].trim()
                  if (firstLine && firstLine.length <= 100) {
                    ticketStmts.updateAgentTitle.run({ $agentTitle: firstLine, $updatedAt: Date.now(), $id: ticket.id })
                  }
                  break
                }
              }
    ...
    case 'Notification': {
      ...
      if (message) {
        if (!ticket.agentTitle && isLikelyTitle(message)) {
          ticketStmts.updateAgentTitle.run({ $agentTitle: message, $updatedAt: Date.now(), $id: ticket.id })
    ...
    case 'TaskCreated': {
      ...
      if (typeof taskTitle === 'string' && taskTitle) {
        ticketStmts.updateAgentTitle.run({ $agentTitle: taskTitle, $updatedAt: Date.now(), $id: ticket.id })
        const updated = ticketStmts.get.get(ticket.id)
        if (updated) broadcastNotification({ type: 'ticket-updated', ticket: updated })
      }
      return c.json({ ok: true })
