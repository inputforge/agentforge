...
export class GitWorktreeManager {
  ...
  async getDiff(worktreePath: string, baseBranch: string): Promise<DiffResult> {
    ...
    const raw = await worktreeGit.diff([`origin/${baseBranch}...${currentBranch.trim()}`, '--stat=9999'])
    const rawFull = await worktreeGit.diff([`origin/${baseBranch}...${currentBranch.trim()}`])

    return parseDiff(rawFull, raw)
  }

  async rebase(worktreePath: string, baseBranch: string): Promise<{ success: boolean; conflicted: boolean }> {
    const worktreeGit = simpleGit(worktreePath)
...
function parseDiff(raw: string, stat: string): DiffResult {
