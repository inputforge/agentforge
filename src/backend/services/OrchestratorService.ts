...
function buildCommand(agentType: AgentType, customCommand: string | undefined, description: string): string {
...
export class OrchestratorService {
  ...
  private getGitManager(): GitWorktreeManager | null {
    const config = remoteStmts.get.get() as { repoUrl: string; baseBranch: string; localPath: string } | null
  ...
  async spawnAgent(ticketId: string, agentType: AgentType, customCommand?: string): Promise<void> {
    const ticket = ticketStmts.get.get(ticketId) as { id: string; title: string; description: string } | null
    ...
    try {
      agentProcessManager.spawn(
        ...
        (id, exitCode) => {
          ...
          if (exitCode === 0) {
            ticketStmts.updateStatus.run({ $status: 'review', $updatedAt: Date.now(), $id: ticketId })
            const t = ticketStmts.get.get(ticketId)
            if (t) this.broadcast({ type: 'ticket-updated', ticket: t })
            this.broadcast({
              type: 'notification',
      ...
      agentStmts.updateStatus.run({ $id: agentId, $status: 'error', $needsInput: 0, $endedAt: Date.now() })
      this.broadcast({
        type: 'notification',
        notification: { type: 'error', message: `Failed to spawn agent: ${msg}`, ticketId, agentId },
      })
      throw err
    }
  }
